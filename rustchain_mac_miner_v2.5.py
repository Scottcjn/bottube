...
# Attempt to import requests; fall back to urllib if unavailable (e.g. no SSL)
HAS_REQUESTS = False
try:
    import requests
    HAS_REQUESTS = True
...
if not HAS_REQUESTS:
    ...
    import urllib.parse
    import http.client
    print("[INFO] 'requests' module not available - using http.client fallback (HTTP only)")

    class _UrllibResponse:
        """Minimal requests.Response-like wrapper around urllib responses."""
    ...
    class requests:  # noqa: F811 - shim module
        """Drop-in shim: GET uses urllib, POST uses raw http.client.

        Tiger Python 3.7's urllib has a known issue where POST
        can silently degrade to GET on repeated calls.  Using
        http.client directly gives us explicit control over the
        HTTP method string — no magic, no get_method() overrides.
        """
        class exceptions:
            SSLError = OSError
        ...
        def get(url, **kwargs):
            ...
            try:
                ...
                return _UrllibResponse(resp.getcode(), body)
        ...
        @staticmethod
        def post(url, **kwargs):
            """POST using raw http.client — bypasses urllib entirely.

            Tiger Python 3.7's urllib ignores method="POST" overrides
            on subsequent calls.  http.client.request("POST", ...) is
            explicit and reliable on every Python version.
            """
            timeout = kwargs.get("timeout", 15)
            ...
            try:
                ...
                # Wrap connection errors similar to urllib.error.HTTPError
                if hasattr(e, 'code'):
                    return _UrllibResponse(e.code, str(e))
                raise
...
try:
    ...
    print(warning("[WARN] fingerprint_checks unavailable ({}) - attestation without fingerprint".format(exc)))
...
MINER_VERSION = "2.5.0"
NODE_URL = os.environ.get("RUSTCHAIN_NODE", os.environ.get("NODE_URL", "http://50.28.86.131:8088"))
...
class MacMiner:
    ...
    def _detect_sleep_wake(self):
        ...
        # Need a large gap to avoid false positives on slow hardware
        # (G4 fingerprint checks alone take 15-20 seconds)
        # Skip detection in first 120 seconds (startup grace period)
        if now - self._start_time < 120:
            return False
        if gap > max(LOTTERY_CHECK_INTERVAL * 6, 90):
            return True
        return False
