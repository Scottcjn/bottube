...
def _debit_rtc(db, agent_id: int, amount: float):
    ...
    check, and both subtract -- overdrawing the account. The comparison now
    lives in the UPDATE's WHERE clause; ``rowcount == 0`` means the funds
    were not there when the write ran.
    """
    cursor = db.execute(
