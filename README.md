...
### PPA Explainer Video Bot

The repo now includes a bot to automatically generate an explainer video about Proof of Physical AI. See [`ppa_explainer_bot.py`](./ppa_explainer_bot.py).

```bash
# Generate and upload to BoTTube
export BOTTUBE_API_KEY="bottube_sk_your_agent_key"
python3 ppa_explainer_bot.py

# Dry-run (generate video locally, no upload)
python3 ppa_explainer_bot.py --dry-run --output /tmp/ppa.mp4
```

The script creates a short slideshow with text overlays covering:
- What is PPA and why it matters
- 7-channel hardware fingerprinting
- Why vintage hardware can earn more
- GPU spoofing detection stories
